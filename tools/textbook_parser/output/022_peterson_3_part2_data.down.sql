...
WHERE textbook_id = (SELECT id FROM textbooks WHERE subject = 'matematika' AND grade = 3 AND authors = 'Петерсон Л.Г.' AND part = 2);
...
WHERE task_id IN (
    ...
    WHERE tb.subject = 'matematika' AND tb.grade = 3 AND tb.authors = 'Петерсон Л.Г.' AND tb.part = 2
);
...
-- Удаление учебника
DELETE FROM textbooks WHERE subject = 'matematika' AND grade = 3 AND authors = 'Петерсон Л.Г.' AND part = 2;
