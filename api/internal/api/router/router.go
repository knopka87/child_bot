...
func registerAttemptRoutes(mux *http.ServeMux, h *handler.AttemptHandler) {
	mux.HandleFunc("POST /api/attempts", h.Create)
	mux.HandleFunc("GET /api/attempts/unfinished", h.GetUnfinished)
	mux.HandleFunc("GET /api/attempts/recent", h.GetRecent)
	mux.HandleFunc("POST /api/attempts/{id}/images", h.UploadImage)
	mux.HandleFunc("POST /api/attempts/{id}/process", h.Process)
	mux.HandleFunc("GET /api/attempts/{id}/result", h.GetResult)
	mux.HandleFunc("POST /api/attempts/{id}/next-hint", h.NextHint)
	mux.HandleFunc("DELETE /api/attempts/{id}", h.Delete)
}

// registerHomeRoutes регистрирует routes для home
func registerHomeRoutes(mux *http.ServeMux, h *handler.HomeHandler) {
	mux.HandleFunc("GET /api/home/{childProfileId}", h.GetHomeData)
}
...
func registerProfileRoutes(mux *http.ServeMux, h *handler.ProfileHandler) {
	mux.HandleFunc("POST /api/profiles/child", h.CreateChild)
	mux.HandleFunc("GET /api/profiles/by-platform", h.GetByPlatform)
	mux.HandleFunc("GET /api/profile", h.Get)
	mux.HandleFunc("PUT /api/profile", h.Update)
	mux.HandleFunc("GET /api/profile/history", h.GetHistory)
	mux.HandleFunc("GET /api/profile/stats", h.GetStats)
}
...
func registerAchievementRoutes(mux *http.ServeMux, h *handler.AchievementHandler) {
	mux.HandleFunc("GET /api/achievements", h.List)
	mux.HandleFunc("GET /api/achievements/unlocked", h.GetUnlocked)
	mux.HandleFunc("GET /api/achievements/stats", h.GetStats)
	mux.HandleFunc("GET /api/achievements/has-new", h.HasNew)
	mux.HandleFunc("POST /api/achievements/mark-viewed", h.MarkViewed)
	mux.HandleFunc("GET /api/achievements/{id}", h.GetByID)
	mux.HandleFunc("POST /api/achievements/{id}/claim", h.Claim)
}
...
func registerVillainRoutes(mux *http.ServeMux, h *handler.VillainHandler) {
	mux.HandleFunc("GET /api/villains", h.List)
	mux.HandleFunc("GET /api/villains/active", h.GetActive)
	mux.HandleFunc("GET /api/villains/{id}", h.GetByID)
	mux.HandleFunc("GET /api/villains/{id}/battle", h.GetBattle)
	mux.HandleFunc("GET /api/villains/{id}/victory", h.GetVictory)
	mux.HandleFunc("POST /api/villains/{id}/damage", h.DealDamage)
}
...
func registerSubscriptionRoutes(mux *http.ServeMux, h *handler.SubscriptionHandler) {
	mux.HandleFunc("GET /api/subscription/status", h.GetStatus)
	mux.HandleFunc("GET /api/subscription/plans", h.GetPlans)
	mux.HandleFunc("POST /api/subscription/subscribe", h.Subscribe)
	mux.HandleFunc("DELETE /api/subscription/cancel", h.Cancel)
	mux.HandleFunc("POST /api/subscription/resume", h.Resume)
}
...
func registerReferralRoutes(mux *http.ServeMux, h *handler.ReferralHandler) {
	...
	mux.HandleFunc("POST /api/friends/invite", h.Invite)
	mux.HandleFunc("GET /api/friends/referrals", h.GetReferralData)
	mux.HandleFunc("GET /api/friends/leaderboard", h.GetLeaderboard)
}

// registerAvatarRoutes регистрирует routes для avatars
func registerAvatarRoutes(mux *http.ServeMux, h *handler.AvatarHandler) {
	mux.HandleFunc("GET /api/avatars", h.GetAll)
}
...
func registerConsentRoutes(mux *http.ServeMux, h *handler.ConsentHandler) {
	mux.HandleFunc("POST /api/consent", h.SaveConsent)
	mux.HandleFunc("GET /api/consent", h.GetConsent)
	mux.HandleFunc("GET /api/consent/check", h.CheckConsent)
	mux.HandleFunc("GET /api/consent/history", h.GetConsentHistory)
}
...
func registerAnalyticsRoutes(mux *http.ServeMux, h *handler.AnalyticsHandler) {
	mux.HandleFunc("POST /api/analytics/events", h.SendEvents)
}
...
func registerLegalRoutes(mux *http.ServeMux, h *handler.LegalHandler) {
	mux.HandleFunc("GET /api/legal/privacy", h.GetPrivacyPolicy)
	mux.HandleFunc("GET /api/legal/terms", h.GetTermsOfService)
}
...
func registerEmailRoutes(mux *http.ServeMux, h *handler.EmailHandler) {
	mux.HandleFunc("POST /api/email/verify/send", h.SendVerification)
	mux.HandleFunc("POST /api/email/verify/check", h.VerifyCode)
	mux.HandleFunc("GET /api/email/verify/status", h.CheckVerification)
}

// registerReportRoutes регистрирует routes для отчётов
func registerReportRoutes(mux *http.ServeMux, h *handler.ReportHandler) {
	mux.HandleFunc("GET /api/reports/{childProfileId}/weekly/data", h.GetWeeklyData)
	mux.HandleFunc("GET /api/reports/{childProfileId}/weekly/html", h.GetWeeklyHTML)
	mux.HandleFunc("GET /api/reports/{childProfileId}/list", h.GetReportsList)
	mux.HandleFunc("GET /api/reports/{childProfileId}/{reportDate}/html", h.GetReportByDate)
	mux.HandleFunc("GET /api/reports/{childProfileId}/{reportDate}/download", h.DownloadReportPDF)
	mux.HandleFunc("POST /api/reports/{childProfileId}/generate", h.GenerateReport)
	mux.HandleFunc("GET /api/reports/{childProfileId}/settings", h.GetReportSettings)
	mux.HandleFunc("PUT /api/reports/{childProfileId}/settings", h.UpdateReportSettings)
	mux.HandleFunc("POST /api/reports/{childProfileId}/send-test", h.SendTestReport)
}

// registerCSRFRoutes регистрирует routes для CSRF
func registerCSRFRoutes(mux *http.ServeMux, h *handler.CSRFHandler) {
	mux.HandleFunc("GET /api/csrf-token", h.GetToken)
}
...
func registerWebhookRoutes(mux *http.ServeMux, h *handler.VKPayWebhookHandler) {
	mux.HandleFunc("POST /api/webhooks/vk-pay", h.HandleWebhook)
}
